""" Implementation of Tri-stage learning rate scheduler
    
The original code is taken from Nik Vaessen's git project (nikvaessen/w2v2-speaker).

- TriStageLearningRateLambdaLRFunction.__init__()
    * while defining "self.decay_stage_space", line "steps=self.decay_stage_steps + 2" causes "Index Error (out of list)". Exception on max_step = 859, 1717, ...
    * therefore "steps=self.decay_stage_steps + 2" is modified to "steps=self.decay_stage_steps + 3"

[Code Reference]
- TriStageLearningRateLambdaLRFunction: https://github.com/nikvaessen/w2v2-speaker/blob/master/src/optim/schedule/tri_stage.py
"""
################################################################################
#
    ...
    def __init__(
        ...
        self.decay_stage_space = (
            t.logspace(
                ...
                # steps=self.decay_stage_steps + 2,    # -- caused 'Index Error (out of list)' when max_step = 859, 1717, ...
                steps=self.decay_stage_steps + 3,
                base=math.e,
            )
            .cpu()
            .numpy()
