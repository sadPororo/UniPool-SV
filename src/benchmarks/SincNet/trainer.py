""" This code reproduces the training framework of SincNet,

[Paper Reference]
- Mirco Ravanelli, Yoshua Bengio. 
    "Speaker recognition from raw waveform with sincnet." in IEEE Spoken Language Technology Workshop (2021): 1021-1028.
    doi: 10.1109/SLT.2018.8639585

[Code Reference]
    https://github.com/mravanelli/SincNet/blob/master/speaker_id.py
    
[Summary]
- Optimizer
    * RMSprop optimizer is adopted as the code reference.
    * lr, alpha, & eps are referred from the paper.

- Evaluation method
    * For a fair comparison and assessing the general performance of the network architecture, we take the 'd-vector' way of evaluation, mentioned in the paper, 
      with the cosine similarity measure.
    * The evaluation code is modified from M. Ravanelli's git project, to sample the fixed number of audio slices, referring to the below.
        https://github.com/clovaai/voxceleb_trainer/blob/master/SpeakerNet.py#L136
        https://github.com/clovaai/voxceleb_trainer/blob/master/DatasetLoader.py#L171 & #L26
"""
#%%
import os
import time
...
def evaluate(config:dict, model:nn.Module, loader:DataLoader, device_id:int, threshold=None):
    ...
    # setups for evaluation phase in SincNet
    if is_ddp: in_win_sz = model.module.input_window_size
    else: in_win_sz = model.input_window_size
    nb_eval_samples = config['model']['nb_eval_samples']
    in_win_shift    = int(config['common']['sr'] * config['model']['cw_shift'] / 1000.0)
    ...
    with torch.no_grad():
        ...
        for i, (waveform0, waveform1, label) in enumerate(tqdm(loader)):
            ...
            embedding_list = []
            for wave in [waveform0, waveform1]:
                wav_len = wave.size(-1)
                wave = wave.squeeze(0)
                
                # pad the shortage and batchfy
                if wav_len <= in_win_sz: 
                    wave = torch.from_numpy(np.pad(wave, (0, in_win_sz - wav_len), mode='wrap')).unsqueeze(0) # (1, in_win_sz)
                    
                # slice fixed number of samples in uniform interval
                else:
                    if nb_eval_samples > 0:
                        chunks = []
                        for start_idx in np.linspace(0, wav_len-in_win_sz, num=nb_eval_samples):
                            chunks.append(wave[int(start_idx):int(start_idx)+in_win_sz])
                        ...
                        chunks = []
                        while end_idx < wav_len:
                            chunks.append(wave[start_idx:end_idx])
                            start_idx += in_win_shift
                            end_idx = start_idx + in_win_sz

                        if end_idx >= wav_len:
                            wave = torch.from_numpy(np.pad(wave, (0, end_idx - wav_len), mode='wrap'))
                            chunks.append(wave[start_idx:end_idx])
                        
                    wave = torch.stack(chunks, dim=0)
                
                # chunks to cuda device & model forward
                embedding_list.append(model(wave.to(device_id)))
            
            # final similarity score: mean of pair-wise cosine similarities between audio slices to slices
            sim_mat = (torch.matmul(embedding_list[0], embedding_list[1].T)).clamp(0, 1).detach().cpu().numpy() # range in [0, 1]
            # sim_mat = (torch.matmul(embedding_list[0], embedding_list[1].T)).detach().cpu().numpy() * .5 + .5 # range in [0, 1]
            sim_scr = np.mean(sim_mat)
...
def train(config:dict, logger:Optional[neptune.Run]=None, device_id:int=0):
    ...
    - Mirco Ravanelli, Yoshua Bengio. "Speaker recognition from raw waveform with sincnet." in IEEE Spoken Language Technology Workshop (2021): 1021-1028.
        doi: 10.1109/SLT.2018.8639585
    ...
    printlog('...initiating datasets.\n' +
             f"\t- in SincNet, some of hyper-parameters will be overrided.\n" +
             f"\t\t* model configuration 'cw_len': {config['model']['cw_len']} (ms) will override the configuraion 'train_max_sec'.\n" +
    ...
    config['common']['train_max_sec'] = config['model']['cw_len'] / 1000.0
    config['common']['fix_to_maxlen'] = True
    if is_neptune and is_master:
        logger['parameters/common/train_max_sec'].pop()
    ...
    printlog('...setting optimizer/scheduler.\n' \
             ...
             '\t\t* https://github.com/mravanelli/SincNet/tree/master', logging_path, is_master)
    optimizer = optim.RMSprop(model.parameters(), lr=config['model']['lr'], alpha=config['model']['alpha'], eps=config['model']['eps'])
    printlog(f'\t- optimizer:\n' +
             f"\t\tRMSprop ( lr: {config['model']['lr']}, alpha: {config['model']['alpha']}, eps: {config['model']['eps']} )", logging_path, is_master)
    scheduler = LrScheduler.ConstantLR(optimizer=optimizer, factor=1, total_iters=0)
    printlog(f'\t- scheduler: \n' +
             f'\t\tThe original paper had no scheduler in training process.\n' +
             f'\t\tHowever to count the number of updates, we adopt Constan\n' +
             f'\t\ttLR() scheduler with factor=1, whilch will keep the lear\n' +
              '\t\tning rate constant.', logging_path, is_master)
    ...
    for cur_eval_no in range(start_eval_no, last_eval_no+1):
        ...
        if is_skiptest:
            ...
            val_threshold = valid_scr['EER_threshold'] if is_master else None
            test_time, test_scr = evaluate(config, model, test_loader, device_id, threshold=val_threshold)
        ...
        if is_skiptest:
            ...
            printlog('\t--------------------------------------------------------------------------------------\n' +
                    ...
                    "\t        | FAR*:   {:.04f}, threshold: {:.03f}".format(test_scr['FAR'].item(), val_threshold)         + " |          FAR*:   {:.04f}, threshold: {:.03f}\n".format(best_test_scr['FAR*'], best_test_scr['EER_threshold']) + 
